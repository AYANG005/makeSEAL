...
size_t order_mod_2N(uint64_t x, const seal::Modulus& mod_2N)
{
	...
	for (size_t i = 0; i < log2_exact(mod_2N.value()); ++i) {
		if (current == 1) {
			...
			current = seal::util::multiply_uint_mod(current, current, mod_2N);//Note that this works since 2N has mult subgroup of order N, which is a power of 2, so <p> has to have order power of 2.
		}
	}
	assert(false);
	return 0;
...
void SlotRing::init_slot_modulus(poly base_unit_vector)
{
	...
	a[0] = seal::util::sub_uint_mod(a[0], 1, mod_pe);//Returns (operand1 - operand2) mod modulus.
	poly factor = std::get<2>(poly_eea(a, f, mod_pe));//extended euclidean algo, returns 3rd entry of output which is the gcd of a,f
	assert(factor[d] != 0);
	...
	factor[d] = 0;
	poly_normalize(factor); //cuts the vector down to size by removing all zeros at the back
...
SlotRing SlotRing::change_exponent(uint64_t new_exp) const
{
	...
	poly new_base_unit_vector;
	for (uint64_t x : unit_vectors[0]) {// unit_vectors[0] is the unit vector below. unit_vectors is generated above using the base unit vector and automorphisms.
...
std::shared_ptr<SlotRing> p_257_test_parameters()
{
	...
	return std::make_shared<SlotRing>(SlotRing(16, 257, 3, std::move(unit_vector)).change_exponent(2)); //log2N, p, e, base unit vector. Note change exponent is originally change_exponent(2)
}
